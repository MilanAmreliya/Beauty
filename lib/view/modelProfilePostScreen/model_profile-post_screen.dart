...
class ModelProfilePostScreen extends StatefulWidget {
  final String previousRoute;
  final String singlePostScreen;

  const ModelProfilePostScreen(
      {Key key, this.previousRoute, this.singlePostScreen})
      : super(key: key);
  @override
  _ModelProfilePostScreenState createState() => _ModelProfilePostScreenState();
}

class _ModelProfilePostScreenState extends State<ModelProfilePostScreen> {
  BottomBarViewModel _barController = Get.find();
  ArtistProfileViewModel artistProfileViewModel = Get.find();
  SelectedServiceModel _selectedServiceModel = SelectedServiceModel();
  ...
  void initState() {
    ...
    modelProfileViewModel.getFollowingFollowers(
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: Text(
                              selectedPage.value == 1 ? "Shop" : "Post",
                              style: TextStyle(
                                  fontFamily: "Poppins",
                                  color: Colors.black,
                ...
                return Container(
                    child: GridView.builder(
                        ...
                        itemBuilder: (context, index) {
                          return InkWell(
                            onTap: () async {
                              var serviceId = response.data[index].id;
                              response.data.forEach((element) {
                                if (element.id == serviceId) {
                                  _selectedServiceModel.price = element.price;
                                  _selectedServiceModel.image = element.image;
                                  _selectedServiceModel.id =
                                      element.id.toString();
                                  _selectedServiceModel.endTime =
                                      element.endTime;
                                  _selectedServiceModel.startTime =
                                      element.startTime;
                                  _selectedServiceModel.description =
                                      element.description;
                                  _selectedServiceModel.artistId =
                              ...
                              favoriteStatus = checkFavorite(
                                  response.data[index].id.toString());
                              print("id___>${response.data[index].id}");
                              ...
                              serviceItemDialog(
                                  ...
                                  serviceCategoryName: response.data[index]
                                      .serviceCategory.serviceCategoryName,
                                  rating: response.data[index].serviceRating
                                          ?.toDouble() ??
                                      0.0,
                ...
                child: Container(
                  child: StaggeredGridView.countBuilder(
                    ...
                    itemBuilder: (BuildContext context, int index) => InkWell(
                      onTap: () {
                        onTapCallSinglePost(response, index);
                      },
                      child: ClipRRect(
                        borderRadius: BorderRadius.circular(10),
                        child: Container(
                          ...
                          child: response.data[index].feedImage.isEmpty
                              ? imageNotFound()
                              : response.data[index].feedImage[0].path ==
                                          null ||
                                      response.data[index].feedImage[0].path ==
                                          ''
                                  ? imageNotFound()
  ...
  Widget userProfile() {
    return GetBuilder<ArtistProfileViewModel>(
      builder: (controller) {
        ...
        ArtistProfileDetailResponse response =
            controller.artistProfileApiResponse.data;
        followersCount.value = response.data.followerCount;
        return Column(
          children: [
            ...
            Row(
              children: [
                Spacer(),
                Obx(() {
                  int count = response.data.followerCount;
                  dev.log("COUNT $count");
                  dev.log("COUNT STATUS ${followersCount.value}");
                  return userData("Follower", '${followersCount.value}');
                }),
                /* Spacer(),
  ...
  Widget followButton() {
    return Row(
      children: [
        ...
        SizedBox(),
        GetBuilder<ModelProfileViewModel>(
          builder: (controller) {
            if (controller.followingFollowersApiResponse.status ==
              ...
              return customBtn(
                  ...
                  width: Get.width * 0.3,
                  title: response.followers.indexWhere((element) =>
                              element.id == PreferenceManager.getArtistId()) !=
                          -1
                      ? "Following"
                      : "Follow",
                  radius: 35,
  ...
  Future<void> onTap() async {
    ...
    requestModel.followingId = _barController.selectedArtistId.value.toString();
    requestModel.isFollowing = '0';
    await homeTabViewModel.createFollowPerson(requestModel);
    if (homeTabViewModel.followPersonApiResponse.status == Status.COMPLETE) {
      PostSuccessResponse res = homeTabViewModel.followPersonApiResponse.data;
      if (!res.success) {
        ...
        followCollection
            .doc(_barController.selectedArtistId.value.toString())
            .set({
          'userId': _barController.selectedArtistId.value.toString(),
          'date': DateTime.now()
        }).catchError((e) => print('Add follow data in firebase error :$e'));
        followCollection.doc(PreferenceManager.getArtistId().toString()).set({
          'userId': PreferenceManager.getArtistId().toString(),
          'date': DateTime.now()
        }).catchError((e) => print('Add follow data in firebase error :$e'));
  ...
  Future<void> onTapCallSinglePost(
      ArtistPostResponse response, int index) async {
    ///check like or not
    CommentAndLikePostViewModel likeViewModel = Get.find();
    await likeViewModel.getCheckPostLiked(
      response.data[index].id.toString(),
    ...
    _bottomBarViewModel.setSelectedRoute(widget.singlePostScreen);
